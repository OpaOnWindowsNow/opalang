...
MYOCAMLBUILDEXE=$BUILD_DIR/myocamlbuild.$EXT_EXE
...
then
  echo Generate myocamlbuild
  build/gen_myocamlbuild.sh
else
  echo $MYOCAMLBUILD exists
fi
...
MYOCAMLBUILD_FLAGS=-no-plugin
if [ "$WINDOWS" = "true" ]; then
    WINTOOLS="$PWD/ms_windows"
    MYOCAMLBUILD_FLAGS="$MYOCAMLBUILD_FLAGS -classic-display -j 1 -ocamlc $WINTOOLS/windows_ocamlc -ocamlopt $WINTOOLS/windows_ocamlopt -ocamldep $WINTOOLS/windows_ocamldep"
    export OCAMLLIB=""
    export LIB_ULEX_DIR #=$LIB_ULEX_DIR
    export OCAML_TARGET_DIR #"=$OCAML_TARGET_DIR
fi

if [ -n "$ARGS" ]; then
    msg "bld: ocamlbuild $ARGS"
    $BUILD_DIR/myocamlbuild -no-plugin $MYOCAMLBUILD_FLAGS $ARGS
fi
